...
type CampaignStats struct {
	...
	AttachmentClicked int64 `json:"attachment_opened"` //ADDEDFORNEWFEATURE
...
func getCampaignStats(cid int64) (CampaignStats, error) {
	...
	//INI UNTUK ATTACHMENT YAAA
	query.Where("attach=?", true).Count(&s.AttachmentClicked) //ADDEDFORNEWFEATURE
	if err != nil {
		return s, err
	}
	...
	// INI YANG BARU MASI BELUM SELESAI, UNTUK SEKARANG TIDAK JADI UNTUK DIPAKAI

	// s.OpenedEmail += s.AttachmentClicked
	// err = query.Where("attach=?", EventSent).Count(&s.EmailsSent).Error
	// if err != nil {
	// 	return s, err
	// }

	// INI MASI BLUM SELESAI INGETT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
func GetCampaignMailContext(id int64, uid int64) (Campaign, error) {
	...
	InitializeMongoDB()
	// collection := MongoClient.Database("Result").Collection("campaigns")
	// smtpCollection := MongoClient.Database("Result").Collection("smtp")
	...
	// err := collection.FindOne(context.Background(), bson.M{"id": id, "user_id": uid}).Decode(&c)
	// if err != nil {
	// 	return c, err
	// }

	// // Find the SMTP document associated with the campaign
	// err = smtpCollection.FindOne(context.Background(), bson.M{"id": c.SMTPId}).Decode(&c.SMTP)
	// if err != nil {
	// 	return c, err
	// }
...
func PostCampaign(c *Campaign, uid int64) error {
	...
	for i, g := range c.Groups {
		c.Groups[i], err = GetGroupByName(g.Name, 1)
		if err == gorm.ErrRecordNotFound {
			log.WithFields(logrus.Fields{
				"group": g.Name,
	...
	//INI UNTUK MONGODB CAMPAIGN
	// campaignz := CampaignTest{
	// 	Id:            50,
	// 	UserId:        uid,
	// 	Name:          c.Name,
	...
	var campaignz CampaignTest
	campaignz.Id = 50
	campaignz.UserId = uid
	campaignz.Name = c.Name
	campaignz.CreatedDate = c.CreatedDate
	campaignz.LaunchDate = c.LaunchDate
	campaignz.SendByDate = c.SendByDate
	campaignz.CompletedDate = c.CompletedDate
	campaignz.TemplateId = c.TemplateId
	campaignz.PageId = c.PageId
	campaignz.Status = c.Status
	campaignz.SMTPId = c.SMTPId
	campaignz.URL = c.URL
	...
	var campaignzz CampaignTest
	err = collection.FindOne(context.Background(), bson.D{}, optionsz).Decode(&campaignzz)
	...
	if err == nil {
		campaignz.Id = campaignzz.Id + 1
	} else {
		campaignz.Id = 1 // If there are no documents, start with id 1
	}
	// Insert into the DB

	result, err := collection.InsertOne(context.Background(), campaignz)
	...
	for _, g := range c.Groups {
		...
		for _, t := range g.Targets {
			...
			optionsz := options.FindOne().SetSort(bson.D{{"id", -1}})
			var resultzz ResultTest
			err = collection.FindOne(context.Background(), bson.D{}, optionsz).Decode(&resultzz)
			if err != nil && err != mongo.ErrNoDocuments {
				log.Error(err)
				return err
			}
			...
			// Set p.Id to the highest id + 1
			if err == nil {
				resultz.Id = resultzz.Id + 1
			} else {
				resultz.Id = 1 // If there are no documents, start with id 1
			}

			result, err := collection.InsertOne(context.Background(), resultz)
			...
			MailCollection := MongoClient.Database("Result").Collection("mail_logs")

			result, err = MailCollection.InsertOne(context.Background(), mailz)
